...
void add_label(const char *name, int addr)
{
  ...
  strncpy(new_label->name, name, MAX_LABEL_LEN - 1);
  new_label->name[MAX_LABEL_LEN - 1] = '\0';
 ...
 * @param clean The trimmed assembly line to parse. This buffer may be modified in place.
 ...
 * Adds the label to the label table with its corresponding byte address. If a label is
 * found and processed, the instruction part of the line (if any) is written back into
 * the same buffer pointed to by @clean (after trimming whitespace).
 *
 * Exits on error conditions:
 * - Memory allocation failures
 * - Invalid label names (non-alphanumeric characters except '_')
 *
 * @return 1 if a label definition was found and processed (and @clean was updated to
 *         contain the instruction part after the label), or 0 if no label definition
 *         was found (in which case @clean is left unchanged with respect to labels).
 */
int check_label_definition(char *clean, int current_byte)
{
  ...
  // Scan to see if the line starts with a valid label (Alphanumeric + '_')
  *colon = '\0';
  ...
  // Enforce maximum label length to avoid silent truncation in add_label
  if (strlen(clean) >= MAX_LABEL_LEN)
  {
    fprintf(stderr,
            "Error: Label name '%s' is too long (maximum %d characters)\n",
            clean, MAX_LABEL_LEN - 1);
    exit(1);
  }
...
int parse_instructions(FILE *in)
{
  ...
  while (fgets(line, sizeof(line), in))
  {
    ...
    char mnemonic[16];
    char arg_str[64] = {0};
    int items = sscanf(clean, "%15s %63s", mnemonic, arg_str);
    ...
    // Determine whether the instruction expects an argument and validate it
 ...
 *
 * Uses a single pass over the input file (via parse_instructions) to collect
 * labels and parse instructions into an in-memory list, then iterates that list
 * to generate bytecode with deferred resolution of label references. Supports
 * mnemonics with optional arguments and label references. Exits on parsing or
 * validation errors.
 *
